...
(defn nucleotide-view [nucleotide]
  (dom/li #js {:className nucleotide} nucleotide))

(defn sequence-view [sequence]
  (dom/li nil
    (apply dom/ul nil (map nucleotide-view sequence))))

(defn first-panel-view [sequences owner]
  (reify
    om/IRender
    (render [this]
      (dom/ul #js {:className "sequence"}
        (map sequence-view sequences)))))

(defn second-panel-view [sequences owner]
...
(defn init! [& args]
  ...
  (om/root
    (fn [app owner]
      (dom/div nil
        (om/build first-panel-view (:sequences app))
        (om/build second-panel-view (:controls app))))
    app-state
